class BookingsController < ApplicationController
 ...
 # before_action :restrict_access, only: [:current]
  ...
  def new
    ...
    @booking = Booking.new(user_id: current_user.uid, room: Room.find_by(name: 'Hubben'), begin_date: date, end_date: date + 2.hours)
  ...
  private
    def create_single_booking
      @booking = Booking.new({user_id: current_user.uid}.merge(booking_params))
      respond_to do |format|
        if @booking.save
